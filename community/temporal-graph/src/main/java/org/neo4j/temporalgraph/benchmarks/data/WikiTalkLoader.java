...
public class WikiTalkLoader extends GraphLoader {
    public static final String DEFAULT_PATH = DATASET_FOLDER + "/wiki-talk_temporal_network/wiki-talk-temporal.txt";
    private final boolean useRealTimestamps = false;

    public WikiTalkLoader(String stringPath) {
        super(stringPath, "WikiTalk");
    }
    ...
    public void loadGraphFromDisk() throws IOException {
        ...
        try (var file = new BufferedReader(new FileReader(stringPath))) {
            ...
            while ((string = file.readLine()) != null) {
                ...
                var timestamp = (useRealTimestamps) ? Long.parseLong(nodeIds[2]) : counter;
                nodeSet.add(from);
                nodeSet.add(to);

                // Create one directed relationship
                var forwardRel = new InMemoryRelationship(counter, from, to, 42, timestamp);
        ...
        if (useRealTimestamps) {
            relationships.sort(Comparator.comparing(InMemoryRelationship::getStartTimestamp));
        } else {
