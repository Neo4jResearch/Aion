...
public class DynamicGlobalAggregationBenchmark {
    ...
    public void setup() throws IOException {
        ...
        idsToNames.put(1, "id");
        store = BenchmarkUtils.createTimeStore(numberOfChanges, namesToIds, idsToNames);

        counter = 0;
        nodeList = BenchmarkUtils.createNodes(operations);
        for (var n : nodeList) {
            n.addProperty("id", counter);
            store.addUpdate(n);
    ...
    @Benchmark
    public void testGlobalAggregation(Blackhole blackhole) throws IOException {
        var step = operations / snapshots;
        var timestamp = step;
        for (int i = 0; i < snapshots; i++) {
            // Get and initialize GlobalAggregation
            ...
            // Consume results
            var result = runningAvg.getResult();
    ...
    @Benchmark
    public void testIncrementalGlobalAggregation(Blackhole blackhole) throws IOException {
        var step = operations / snapshots;
        long timestamp = step;
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(DynamicGlobalAggregationBenchmark.class.getSimpleName())
